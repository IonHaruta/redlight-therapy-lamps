...
const CartPage = () => {
  ...
  const [paynetBusy, setPaynetBusy] = useState(false);
  ...
  const handlePaynet = async () => {
    ...
    setPaynetBusy(true);
    try {
      const res = await fetch("/api/paynet/register", {
        method: "POST",
      ...
      if (!res.ok || !data.checkoutPostUrl || !data.fields) {
        const msg =
          typeof data.error === "string"
            ? data.error
            : res.status === 503
              ? t.cart.paynetNotConfigured
              : t.cart.paynetErrorGeneric;
        toast.error(msg);
        return;
      }
      redirectPostToPaynet({
        checkoutPostUrl: data.checkoutPostUrl,
        fields: data.fields,
      });
    } catch {
            ...
            <div className="mt-2 rounded-lg border border-border bg-card p-6">
              ...
              <div className="mb-4 grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="paynet-fn" className="text-muted-foreground">
                ...
                <div className="space-y-2">
                  <Label htmlFor="paynet-ln" className="text-muted-foreground">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="paynet-email" className="text-muted-foreground">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="paynet-phone" className="text-muted-foreground">
              ...
              <div className="mb-6 flex gap-3">
                <Checkbox
                  ...
                  aria-required
                />
                <label htmlFor="paynet-terms" className="cursor-pointer text-sm leading-snug text-muted-foreground">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <Button
                  type="button"
                  disabled={paynetBusy || !termsAccepted}
                  ...
                  onClick={() => void handlePaynet()}
                >
                  {paynetBusy ? t.cart.paynetBusy : t.cart.checkout}
                </Button>
