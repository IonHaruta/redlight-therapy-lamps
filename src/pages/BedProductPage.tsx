...
import {
  ...
  renderAccessoryDetailCardGrid,
  renderAccessoryModelCompareTable,
} from "@/pages/AccessoryProductPage";
import { bedProducts, getBedBySlug } from "@/data/bed";
...
const MediaPreview = ({ media, name }: { media: MaskMedia; name: string }) => {
  ...
  return (
    <img src={assetUrl(media.path)} alt={media.alt || name} className="h-full w-full object-contain" />
  );
};
...
const BedProductPage = () => {
  ...
  const product = getBedBySlug(slug || "");
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex min-h-[60vh] items-center justify-center px-4 pt-24 text-center">
          <div>
            ...
            <Link to="/pat" className="text-primary hover:underline">
              {ui[locale].back}
            </Link>
          </div>
        </div>
  ...
  const thumbPath = product.media.find((m) => m.type === "image")?.path ?? product.media[0]?.path;
  ...
  const showModelPicker = bedProducts.length > 1;
